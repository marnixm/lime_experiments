...
def main(dataset, algorithm, parameters):
  ...
  printLog(path, 'Average number of flipped predictions:', np.mean(flipped_preds_size), '+-', np.std(flipped_preds_size), '\n')
  printLog(path, 'Precision:')
  results = {'Precision': {}, 'Recall': {}, 'F1': {}}
  for expl in explainer_names:
    printLog(path, expl, np.mean(precision[expl]), '+-', np.std(precision[expl]), 'pvalue', sp.stats.ttest_ind(precision[expl], precision[test_against])[1].round(4))
  ...
  printLog(path, '')
  printLog(path, 'Recall:')
  for expl in explainer_names:
    printLog(path, expl, np.mean(recall[expl]), '+-', np.std(recall[expl]), 'pvalue', sp.stats.ttest_ind(recall[expl], recall[test_against])[1].round(4))
